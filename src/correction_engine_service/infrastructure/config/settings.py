...
class Settings:
    ...
    def __init__(self):
        ...
        self.correction_models = {
            ...
            "grammar_corrector": {
                "model_path": os.getenv("GRAMMAR_MODEL_PATH", "/models/grammar-corrector"),
                "confidence_threshold": float(os.getenv("GRAMMAR_CONFIDENCE_THRESHOLD", "0.8")),
                "max_suggestions": int(os.getenv("GRAMMAR_MAX_SUGGESTIONS", "5"))
            },
            "spell_checker": {
                "model_path": os.getenv("SPELL_MODEL_PATH", "/models/spell-checker"),
                "confidence_threshold": float(os.getenv("SPELL_CONFIDENCE_THRESHOLD", "0.9")),
                "max_suggestions": int(os.getenv("SPELL_MAX_SUGGESTIONS", "3"))
            },
            "style_corrector": {
                "model_path": os.getenv("STYLE_MODEL_PATH", "/models/style-corrector"),
                "confidence_threshold": float(os.getenv("STYLE_CONFIDENCE_THRESHOLD", "0.7")),
        ...
        self.database = {
            "url": os.getenv("DATABASE_URL", "postgresql://user:password@localhost:5432/correction_engine"),
        ...
        # Redis configuration
        self.redis = {
            "url": os.getenv("REDIS_URL", "redis://localhost:6379/2"),  # Different DB from other services
            "max_connections": int(os.getenv("REDIS_MAX_CONNECTIONS", "10")),
            "retry_on_timeout": os.getenv("REDIS_RETRY_ON_TIMEOUT", "true").lower() == "true",
            "cache_ttl": int(os.getenv("REDIS_CACHE_TTL", "1800"))  # 30 minutes default
        }
        ...
        self.processing = {
            ...
            "max_suggestions_per_request": int(os.getenv("MAX_SUGGESTIONS_PER_REQUEST", "10"))
        }
