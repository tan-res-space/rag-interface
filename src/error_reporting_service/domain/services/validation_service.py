...
class ErrorValidationService:
    ...
    def __init__(self):
        ...
        self._critical_categories = {"patient_safety"}

        self._high_priority_categories = {"medical_terminology"}
    ...
    def assess_severity(self, error_report: ErrorReport) -> SeverityLevel:
        ...
        if any(
            category in self._critical_categories
            for category in error_report.error_categories
        ):
    ...
    def validate_error_report_completeness(
    ...
    async def validate_error_report_request(self, request) -> ValidationResult:
        ...
        try:
            ...
            errors.append(
                ValidationError("INVALID_UUID", "Invalid job_id format", "job_id")
            )
        ...
        try:
            ...
            errors.append(
                ValidationError(
                    "INVALID_UUID", "Invalid speaker_id format", "speaker_id"
                )
            )
        ...
        try:
            ...
            errors.append(
                ValidationError(
                    "INVALID_UUID", "Invalid reported_by format", "reported_by"
                )
            )
        ...
        if not request.original_text or not request.original_text.strip():
            errors.append(
                ValidationError(
                    "EMPTY_FIELD", "original_text cannot be empty", "original_text"
                )
            )
        ...
        if not request.corrected_text or not request.corrected_text.strip():
            errors.append(
                ValidationError(
                    "EMPTY_FIELD", "corrected_text cannot be empty", "corrected_text"
                )
            )
        ...
        if request.end_position <= request.start_position:
            errors.append(
                ValidationError(
                    "INVALID_POSITION_RANGE",
                    "end_position must be greater than start_position",
                    "end_position",
                )
            )
        ...
        if not request.error_categories:
            errors.append(
                ValidationError(
                    "EMPTY_FIELD",
                    "error_categories cannot be empty",
                    "error_categories",
                )
            )
        ...
        if request.severity_level not in valid_severities:
            errors.append(
                ValidationError(
                    "INVALID_SEVERITY",
                    f"severity_level must be one of {valid_severities}",
                    "severity_level",
                )
            )
        ...
        if request.context_notes and len(request.context_notes) > 1000:
            errors.append(
                ValidationError(
                    "TEXT_TOO_LONG",
                    "context_notes exceeds maximum length of 1000 characters",
                    "context_notes",
                )
            )
        ...
        if request.original_text and len(request.original_text) > 5000:
            warnings.append(
                ValidationError(
                    "LONG_TEXT",
                    "original_text is very long, consider splitting",
                    "original_text",
                )
            )
    ...
    def validate_position_range(
        ...
        if end_pos > len(text):
            errors.append(
                ValidationError(
                    "OUT_OF_BOUNDS",
                    "position range exceeds text length",
                    "end_position",
                )
            )
    ...
    def validate_custom_categories(self, categories: List[str]) -> ValidationResult:
        ...
        for category in categories:
            if not category or not category.strip():
                errors.append(
                    ValidationError(
                        "EMPTY_CATEGORY", "Category cannot be empty", "categories"
                    )
                )
    ...
    def validate_medical_terminology_context(
        self, error_report: ErrorReport
    ) -> ValidationResult:
        ...
        if "medical_terminology" in error_report.error_categories:
            if (
                error_report.severity_level != SeverityLevel.HIGH
                and error_report.severity_level != SeverityLevel.CRITICAL
            ):
                warnings = [
                    ValidationError(
                        "SEVERITY_MISMATCH",
                        "Medical terminology errors should typically be high or critical severity",
                        "severity_level",
                    )
                ]
    ...
    def validate_duplicate_error(
        self, error_report: ErrorReport, existing_errors: List[ErrorReport]
    ) -> ValidationResult:
        ...
        for existing in existing_errors:
            if (
                existing.job_id == error_report.job_id
                and existing.speaker_id == error_report.speaker_id
                and existing.start_position == error_report.start_position
                and existing.end_position == error_report.end_position
                and existing.original_text == error_report.original_text
            ):
                errors.append(
                    ValidationError(
                        "DUPLICATE_ERROR",
                        "Duplicate error report detected",
                        "error_report",
                    )
                )
                break
    ...
    def validate_error_consistency(self, error_report: ErrorReport) -> ValidationResult:
        ...
        if error_report.original_text == error_report.corrected_text:
            errors.append(
                ValidationError(
                    "TEXT_CONSISTENCY",
                    "Original and corrected text cannot be identical",
                    "corrected_text",
                )
            )

        # Check if the specified error position actually contains an error
        if error_report.start_position < len(
            error_report.original_text
        ) and error_report.end_position <= len(error_report.original_text):
            original_segment = error_report.original_text[
                error_report.start_position : error_report.end_position
            ]
            corrected_segment = error_report.corrected_text[
                error_report.start_position : error_report.end_position
            ]
            ...
            if original_segment == corrected_segment:
                errors.append(
                    ValidationError(
                        "TEXT_CONSISTENCY",
                        "No actual change detected in specified position range",
                        "position",
                    )
                )
    ...
    def validate_context_window(
        self, error_report: ErrorReport, min_context: int = 10
    ) -> ValidationResult:
        ...
        if (
            context_before < min_context
            and context_after < min_context
            and text_length > min_context * 2
        ):
            errors.append(
                ValidationError(
                    "INSUFFICIENT_CONTEXT",
                    "Insufficient context around error for proper analysis",
                    "context",
                )
            )
    ...
    def validate_temporal_consistency(
        ...
        if error_report.reported_at > now:
            errors.append(
                ValidationError(
                    "FUTURE_TIMESTAMP",
                    "Reported timestamp cannot be in the future",
                    "reported_at",
                )
            )
        ...
        if error_report.error_timestamp > error_report.reported_at:
            errors.append(
                ValidationError(
                    ...
                    "Error timestamp cannot be after reported timestamp",
                    "timestamps",
                )
            )
    ...
    def validate_error_batch(
        ...
        for i, error_report in enumerate(error_reports):
            ...
            for issue in completeness_issues:
                all_errors.append(
                    ValidationError(
                        "BATCH_VALIDATION", f"Error {i}: {issue}", f"error_{i}"
                    )
                )
