...
class InMemoryDatabaseAdapter(IDatabaseAdapter):
    ...
    async def find_error_by_id(self, error_id: UUID) -> Optional[ErrorReport]:
        ...
        storage = self._transaction_data if self._transaction_data is not None else self._error_reports
    ...
    async def find_errors_by_speaker(self, speaker_id: UUID, filters: Optional[Dict] = None) -> List[ErrorReport]:
        ...
        for error_report in storage.values():
            if error_report.speaker_id == speaker_id:
                ...
                if filters:
                    if "severity_level" in filters and error_report.severity_level.value != filters["severity_level"]:
                        continue
                    if "status" in filters and error_report.status.value != filters["status"]:
                        continue
                    if "categories" in filters:
                        if not any(cat in error_report.error_categories for cat in filters["categories"]):
                            continue
    ...
    async def search_errors(self, query: Dict[str, Any]) -> List[ErrorReport]:
        ...
        for error_report in storage.values():
            ...
            if "speaker_id" in query and error_report.speaker_id != UUID(query["speaker_id"]):
                match = False
            if "job_id" in query and error_report.job_id != UUID(query["job_id"]):
                match = False
            if "severity_level" in query and error_report.severity_level.value != query["severity_level"]:
                match = False
            if "status" in query and error_report.status.value != query["status"]:
                match = False
            if "text_search" in query:
                search_term = query["text_search"].lower()
                if (search_term not in error_report.original_text.lower() and 
                    search_term not in error_report.corrected_text.lower()):
                    match = False
