...
from ...domain.entities.speaker_rag_processing_job import SpeakerRAGProcessingJob, JobType, JobStatus
from ...domain.services.speaker_rag_processing_service import SpeakerRAGProcessingService
...
class ProcessSpeakerRAGDataUseCase:
        ...
        try:
            ...
            for data_item in request.historical_data_items:
                try:
                    ...
                    if pairs:
                        saved_pairs = await self._speaker_rag_repo.batch_create_error_correction_pairs(pairs)
            ...
            # Generate statistics
            error_statistics = self._rag_service.calculate_speaker_error_frequency(all_pairs)
    ...
    async def vectorize_speaker_error_pairs(
        ...
        job = SpeakerRAGProcessingJob(
            ...
            job_type=JobType.VECTORIZATION
        )
    ...
    async def get_speaker_error_patterns(
        ...
        pairs = await self._speaker_rag_repo.get_error_correction_pairs_by_speaker(speaker_id)
        
        if not pairs:
            return {"speaker_id": str(speaker_id), "error_patterns": {}, "total_pairs": 0}
        ...
        return {
            ...
            "training_suitable_pairs": len([p for p in pairs if p.is_suitable_for_training()])
        }
    ...
    async def get_processing_job_status(self, job_id: UUID) -> Optional[ProcessingJobResponse]:
        """
    ...
    async def _vectorize_pair_batch(
        self,
        pairs: List[SpeakerErrorCorrectionPair],
        speaker_id: UUID
    ) -> None:
        """
    ...
    def _pair_to_response(self, pair: SpeakerErrorCorrectionPair) -> ErrorCorrectionPairResponse:
