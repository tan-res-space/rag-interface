...
from ..entities.speaker_rag_processing_job import SpeakerRAGProcessingJob, JobType
...
class SpeakerRAGProcessingService:
    ...
    def extract_error_correction_pairs(
        ...
        for asr_sent, final_sent, alignment_score in aligned_sentences:
            if asr_sent != final_sent and alignment_score > 0.3:  # Threshold for meaningful alignment
    ...
    def categorize_error_patterns(
        self,
        error_correction_pairs: List[SpeakerErrorCorrectionPair]
        ...
        for pair in error_correction_pairs:
            error_type = self._classify_error_type(pair.error_text, pair.correction_text)
    ...
    def calculate_speaker_error_frequency(
        ...
        for pair in error_correction_pairs:
            ...
            error_counts[error_type]["examples"].append({
                ...
                "confidence": float(pair.confidence_score) if pair.confidence_score else 0.0
            })
        ...
        for error_type, data in error_counts.items():
            ...
            confidences = [ex["confidence"] for ex in data["examples"] if ex["confidence"] > 0]
    ...
    def _extract_context(
        ...
        return context_before if context_before else None, context_after if context_after else None
    ...
    def _is_medical_terminology_error(self, error_text: str, correction_text: str) -> bool:
        """Check if error involves medical terminology."""
        medical_indicators = ["mg", "ml", "cc", "dr", "patient", "diagnosis", "treatment"]
        error_lower = error_text.lower()
        correction_lower = correction_text.lower()
        
        return any(term in error_lower or term in correction_lower for term in medical_indicators)
    ...
    def _is_punctuation_error(self, error_text: str, correction_text: str) -> bool:
        ...
        return bool(re.search(r'[.!?,:;]', error_text) or re.search(r'[.!?,:;]', correction_text))
    ...
    def _create_training_prompt(
        ...
        return {
            ...
            "confidence": str(pair.confidence_score) if pair.confidence_score else "0.0"
        }
