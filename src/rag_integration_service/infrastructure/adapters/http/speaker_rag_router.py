...
from ....application.use_cases.process_speaker_rag_data_use_case import ProcessSpeakerRAGDataUseCase
...
async def get_speaker_rag_use_case() -> ProcessSpeakerRAGDataUseCase:
    ...
    raise HTTPException(status_code=501, detail="Speaker RAG processing use case not implemented")


@router.post("/process-historical", response_model=SpeakerRAGProcessingResponse, status_code=201)
async def process_speaker_historical_data(
    ...
    try:
        ...
        logger.info(f"Historical data processing completed: {response.total_pairs_generated} pairs generated")
        return response
...
@router.post("/generate-pairs", response_model=List[ErrorCorrectionPairResponse], status_code=201)
async def generate_error_correction_pairs(
    ...
    """
    try:
        logger.info(f"Generating error-correction pairs for speaker: {request.speaker_id}")
        ...
        raise HTTPException(status_code=500, detail="Failed to generate error-correction pairs")
...
@router.get("/speaker/{speaker_id}/error-patterns", response_model=SpeakerErrorPatternsResponse)
async def get_speaker_error_patterns(
    ...
    max_examples_per_type: int = Query(5, ge=1, le=20, description="Maximum examples per error type"),
    use_case: ProcessSpeakerRAGDataUseCase = Depends(get_speaker_rag_use_case)
):
    """
...
async def create_processing_job(
    ...
    """
    try:
        logger.info(f"Creating processing job for speaker: {request.speaker_id}, type: {request.job_type}")
...
async def get_processing_job_status(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to get processing job status")
...
async def get_processing_jobs(
    ...
    """
    try:
        logger.info(f"Getting processing jobs with filters: speaker_id={speaker_id}, job_type={job_type}")
