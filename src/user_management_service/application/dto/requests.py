...
class ChangePasswordRequest(BaseModel):
    ...
    change_reason: str = Field(
        ..., pattern=r"^(user_request|admin_reset|policy_expiry)$"
    )
    ip_address: Optional[str] = None
...
class GetUsersRequest(BaseModel):
    ...
    sort_by: str = Field(
        default="created_at", pattern=r"^(username|email|created_at|last_login)$"
    )
...
class SearchSpeakersRequest(BaseModel):
    ...
    def validate_ser_score_range(cls, v, values):
        ...
        if (
            v is not None
            and "min_ser_score" in values
            and values["min_ser_score"] is not None
        ):
            if v < values["min_ser_score"]:
                raise ValueError(
                    "max_ser_score must be greater than or equal to min_ser_score"
                )
        return v
