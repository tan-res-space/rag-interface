...
from ...domain.events.domain_events import UserLoginSuccessEvent, UserLoginFailureEvent, UserAccountLockedEvent
        ...
        try:
            ...
            if not user:
                await self._handle_authentication_failure(
                    request.username, "user_not_found", request.ip_address, request.user_agent
                )
            ...
            if not self._check_account_status(user):
                await self._handle_authentication_failure(
                    request.username, "account_inactive", request.ip_address, request.user_agent, user
                )
                return AuthenticationResponse(
                    success=False,
            ...
            if self._is_account_locked(user):
                await self._handle_authentication_failure(
                    request.username, "account_locked", request.ip_address, request.user_agent, user
                )
                return AuthenticationResponse(
                    success=False,
            ...
            if not await self._verify_password(user, request.password):
                await self._handle_authentication_failure(
                    request.username, "invalid_password", request.ip_address, request.user_agent, user
                )
            ...
            # 7. Publish success event
            await self._publish_login_success_event(user, request.ip_address, request.user_agent)
