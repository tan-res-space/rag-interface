...
from ..dto.responses import BucketTransitionRequestResponse
...
class ManageBucketTransitionsUseCase:
    ...
    async def create_transition_request(
        self, request: CreateBucketTransitionRequest
    ...
    async def get_transition_request_by_id(
        ...
        """
        transition_request = (
            await self._speaker_repo.get_bucket_transition_request_by_id(request_id)
        )
    ...
    async def approve_transition_request(
        self, request_id: UUID, approval_request: ApproveBucketTransitionRequest
    ) -> BucketTransitionRequestResponse:
        """
        ...
        updated_request = await self._speaker_repo.update_bucket_transition_request(
            transition_request
        )

        # Update speaker bucket
        speaker = await self._speaker_repo.get_speaker_by_id(
            transition_request.speaker_id
        )
        if speaker:
    ...
    async def reject_transition_request(
        self, request_id: UUID, rejection_request: RejectBucketTransitionRequest
    ) -> BucketTransitionRequestResponse:
        """
    ...
    async def get_speaker_transition_history(
        self, speaker_id: UUID, limit: int = 50
    ) -> List[BucketTransitionRequestResponse]:
        """
        ...
        """
        transition_history = (
            await self._speaker_repo.get_transition_history_for_speaker(speaker_id)
        )

        # Sort by creation date (most recent first)
        sorted_history = sorted(
            transition_history, key=lambda r: r.created_at, reverse=True
        )[:limit]
    ...
    async def auto_generate_transition_requests(
        self,
    ) -> List[BucketTransitionRequestResponse]:
        """
        ...
        for speaker in speakers_needing_transition:
            ...
            existing_requests = (
                await self._speaker_repo.get_transition_history_for_speaker(speaker.id)
            )
            ...
            if not has_pending:
                ...
                # Save the request
                created_request = (
                    await self._speaker_repo.create_bucket_transition_request(
                        transition_request
                    )
                )
                generated_requests.append(
                    self._transition_request_to_response(created_request)
                )

        return generated_requests

    def _transition_request_to_response(
        self, transition_request: BucketTransitionRequest
    ) -> BucketTransitionRequestResponse:
        """
