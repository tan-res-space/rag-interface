...
class ManageSpeakersUseCase:
    ...
    async def create_speaker(self, request: CreateSpeakerRequest) -> SpeakerResponse:
        ...
        if existing_speaker:
            raise ValueError(f"Speaker with identifier '{request.speaker_identifier}' already exists")
    ...
    async def get_speaker_by_identifier(self, speaker_identifier: str) -> Optional[SpeakerResponse]:
        """
    ...
    async def update_speaker(self, speaker_id: UUID, request: UpdateSpeakerRequest) -> SpeakerResponse:
        """
    ...
    async def search_speakers(self, request: SearchSpeakersRequest) -> SpeakerListResponse:
        """
        ...
        speakers = await self._speaker_repo.search_speakers(
            ...
            min_ser_score=Decimal(str(request.min_ser_score)) if request.min_ser_score else None,
            max_ser_score=Decimal(str(request.max_ser_score)) if request.max_ser_score else None,
    ...
    async def get_speakers_by_bucket(self, bucket: SpeakerBucket) -> List[SpeakerResponse]:
        """
    ...
    async def get_bucket_statistics(self) -> SpeakerBucketStatsResponse:
        ...
        speakers_needing_transition = len(await self._speaker_repo.get_speakers_needing_transition())
        ...
        quality_distribution = {
            "high": bucket_counts.get("no_touch", 0) + bucket_counts.get("low_touch", 0),
