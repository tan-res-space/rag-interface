...
class BucketTransitionRequest:
    ...
    def _validate_bucket_transition(self) -> None:
        ...
        if not self.from_bucket.can_transition_to(self.to_bucket):
            raise ValueError(
                f"Invalid transition from {self.from_bucket} to {self.to_bucket}"
            )
    ...
    def is_improvement_transition(self) -> bool:
        ...
        """
        return (
            self.to_bucket.get_priority_level() > self.from_bucket.get_priority_level()
        )
    ...
    def get_request_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ser_improvement": (
                float(self.ser_improvement) if self.ser_improvement else None
            ),
            "status": self.status,
