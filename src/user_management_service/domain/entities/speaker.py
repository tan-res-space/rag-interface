...
class Speaker:
    ...
    def is_fully_processed(self) -> bool:
        ...
        """
        return (
            self.processed_notes_count == self.total_notes_count
            and self.total_notes_count > 0
        )
    ...
    def get_quality_trend(self) -> str:
        ...
        if (
            recommended_bucket.get_priority_level()
            > self.current_bucket.get_priority_level()
        ):
            return "improving"
        elif (
            recommended_bucket.get_priority_level()
    ...
    def update_statistics(
        self, total_notes: int, processed_notes: int, avg_ser: Optional[Decimal]
    ) -> None:
        """
    ...
    def transition_bucket(self, new_bucket: SpeakerBucket) -> None:
        ...
        if not self.can_transition_to_bucket(new_bucket):
            raise ValueError(
                f"Invalid bucket transition from {self.current_bucket} to {new_bucket}"
            )
    ...
    def get_speaker_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_ser_score": (
                float(self.average_ser_score) if self.average_ser_score else None
            ),
