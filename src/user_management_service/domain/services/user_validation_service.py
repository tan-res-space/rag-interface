...
class UserValidationService:
    ...
    def validate_username(self, username: str) -> List[str]:
        ...
        if len(username) < self.MIN_USERNAME_LENGTH:
            errors.append(
                f"Username must be at least {self.MIN_USERNAME_LENGTH} characters"
            )

        if len(username) > self.MAX_USERNAME_LENGTH:
            errors.append(
                f"Username must be no more than {self.MAX_USERNAME_LENGTH} characters"
            )
    ...
    def validate_password_policy(self, password: str) -> List[str]:
        ...
        if len(password) > self.MAX_PASSWORD_LENGTH:
            errors.append(
                f"Password must be no more than {self.MAX_PASSWORD_LENGTH} characters"
            )
    ...
    def validate_account_security(self, user: User) -> List[str]:
        ...
        if user.account_locked_until and user.account_locked_until > datetime.utcnow():
            ...
            warnings.append(
                f"Account is locked for {time_remaining.seconds // 60} more minutes"
            )

        # Check failed login attempts
        if user.failed_login_attempts >= 3:
            warnings.append(
                f"Account has {user.failed_login_attempts} failed login attempts"
            )
        ...
        if user.last_login:
            ...
            if days_since_login > 30:
                warnings.append(
                    f"Account has not been used for {days_since_login} days"
                )

        return warnings

    def can_user_perform_action(
        self, user: User, required_permission: Permission
    ) -> bool:
        """
