    ...
    async def change_password(self, user_id: str, new_password: str) -> bool:
        ...
        try:
            ...
            if validation_errors:
                logger.warning(f"Password policy validation failed for user {user_id}: {validation_errors}")
                return False
            
            # Check if password is compromised
            if await self.is_password_compromised(new_password):
                logger.warning(f"Attempted to set compromised password for user {user_id}")
                return False
