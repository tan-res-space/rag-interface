...
class InMemoryUserRepositoryAdapter(IUserRepositoryPort):
    ...
    async def find_by_roles(
        self, roles: List[UserRole], limit: int = 100
    ) -> List[User]:
        """Find users by roles"""
    ...
    async def find_by_department(self, department: str, limit: int = 100) -> List[User]:
        ...
        for user in storage.values():
            if (
                ...
                and user.profile.department.lower() == department.lower()
            ):
                matching_users.append(deepcopy(user))
                if len(matching_users) >= limit:
                    break
    ...
    async def search_users(self, search_term: str, limit: int = 20) -> List[User]:
        ...
        for user in storage.values():
            if (
                ...
                or search_lower in user.profile.email.lower()
                or search_lower in user.profile.first_name.lower()
                or search_lower in user.profile.last_name.lower()
            ):
                matching_users.append(deepcopy(user))
                if len(matching_users) >= limit:
                    break
        ...
        logger.debug(
            f"Found {len(matching_users)} users matching search: {search_term}"
        )
        return matching_users
    ...
    async def get_users_paginated(
        ...
        for user in storage.values():
            ...
            # Department filter
            if department and (
                not user.profile.department
                or user.profile.department.lower() != department.lower()
            ):
                continue
        ...
        if sort_by == "username":
            ...
            filtered_users.sort(
                key=lambda u: u.last_login or u.created_at, reverse=reverse
            )
        else:  # created_at
        ...
        logger.debug(
            f"Retrieved {len(paginated_users)} users (page {page}, total {total_count})"
        )
    ...
    async def commit_transaction(self) -> None:
        ...
        if self._transaction_data is not None:
            ...
            self._username_index = {
                user.username: user.user_id for user in self._users.values()
            }
            self._email_index = {
                user.profile.email: user.user_id for user in self._users.values()
            }
