...
from ....application.use_cases.manage_bucket_transitions_use_case import ManageBucketTransitionsUseCase
...
async def get_bucket_transition_use_case() -> ManageBucketTransitionsUseCase:
    ...
    raise HTTPException(status_code=501, detail="Bucket transition use case not implemented")
...
async def create_transition_request(
    ...
    """
    try:
        logger.info(f"Creating bucket transition request for speaker: {request.speaker_id}")
        ...
        raise HTTPException(status_code=500, detail="Failed to create transition request")
...
async def get_transition_requests(
    ...
    """
    try:
        logger.info(f"Getting transition requests with filters: status={status}, urgent_only={urgent_only}")
...
async def get_pending_transition_requests(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to get pending transition requests")
...
async def approve_transition_request(
    ...
    try:
        ...
        response = await use_case.approve_transition_request(request_id, approval_request)
        ...
        raise HTTPException(status_code=500, detail="Failed to approve transition request")
...
async def reject_transition_request(
    ...
    try:
        ...
        response = await use_case.reject_transition_request(request_id, rejection_request)
        ...
        raise HTTPException(status_code=500, detail="Failed to reject transition request")


@router.get("/speaker/{speaker_id}/history", response_model=List[BucketTransitionRequestResponse])
async def get_speaker_transition_history(
    ...
    try:
        ...
        logger.info(f"Found {len(response)} transition records for speaker {speaker_id}")
        return response
...
async def get_transition_statistics():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to get transition statistics")
