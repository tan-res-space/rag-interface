...
async def get_speaker_use_case() -> ManageSpeakersUseCase:
    ...
    raise HTTPException(status_code=501, detail="Speaker management use case not implemented")
...
async def search_speakers(
    name_pattern: Optional[str] = Query(None, description="Speaker name pattern to search for"),
    bucket: Optional[SpeakerBucket] = Query(None, description="Filter by speaker bucket"),
    min_ser_score: Optional[float] = Query(None, ge=0, le=100, description="Minimum SER score filter"),
    max_ser_score: Optional[float] = Query(None, ge=0, le=100, description="Maximum SER score filter"),
    has_sufficient_data: Optional[bool] = Query(None, description="Filter by data sufficiency"),
    ...
    try:
        ...
        logger.info(f"Searching speakers with filters: page={page}, page_size={page_size}")
...
async def get_speakers_needing_transition(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to get speakers needing transition")
...
async def update_speaker_statistics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to update speaker statistics")
