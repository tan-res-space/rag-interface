...
from ..dto.requests import CalculateSERRequest, BatchCalculateSERRequest
from ..dto.responses import SERCalculationResponse, BatchSERCalculationResponse
...
class CalculateSERMetricsUseCase:
    ...
    def execute_batch(self, request: BatchCalculateSERRequest) -> BatchSERCalculationResponse:
        """
        ...
        # Prepare text pairs
        text_pairs = [
            (item.asr_text, item.reference_text)
            for item in request.calculation_items
        ]
        ...
        responses = []
        for i, (item, ser_metrics) in enumerate(zip(request.calculation_items, ser_metrics_list)):
        ...
        return BatchSERCalculationResponse(
            ...
            successful_calculations=len(responses),  # All successful in this implementation
    ...
    def calculate_speaker_average_ser(
        self, 
        speaker_id: UUID, 
        ser_metrics_list: List[SERMetrics]
    ) -> Decimal:
        """
    ...
    def _calculate_summary_statistics(self, ser_metrics_list: List[SERMetrics]) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "high_quality_percentage": (quality_distribution.get("high", 0) / len(ser_metrics_list)) * 100,
            "medium_quality_percentage": (quality_distribution.get("medium", 0) / len(ser_metrics_list)) * 100,
            "low_quality_percentage": (quality_distribution.get("low", 0) / len(ser_metrics_list)) * 100
        }
