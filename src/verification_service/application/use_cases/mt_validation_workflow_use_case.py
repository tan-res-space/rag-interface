...
class MTValidationWorkflowUseCase:
    ...
    async def start_validation_session(
        self, request: StartValidationSessionRequest
    ) -> ValidationSessionResponse:
        """
        ...
        return self._session_to_response(
            updated_session, 0
        )  # 0 completed items initially
    ...
    async def get_validation_test_data(
        ...
        for item in test_data_items:
            ...
            original_ser = self._ser_service.calculate_ser(
                item.original_asr_text, item.final_reference_text
            )

            corrected_ser = self._ser_service.calculate_ser(
                item.rag_corrected_text, item.final_reference_text
            )

            # Calculate improvement metrics
            improvement_metrics = self._ser_service.compare_ser_metrics(
                original_ser, corrected_ser
            )
    ...
    async def submit_mt_feedback(
        self, request: SubmitMTFeedbackRequest
    ) -> MTFeedbackResponse:
        """
        ...
        original_ser = self._ser_service.calculate_ser(
            request.original_asr_text, request.final_reference_text
        )

        corrected_ser = self._ser_service.calculate_ser(
            request.rag_corrected_text, request.final_reference_text
        )

        ser_comparison = self._ser_service.compare_ser_metrics(
            original_ser, corrected_ser
        )
    ...
    async def complete_validation_session(
        self, request: CompleteValidationSessionRequest
    ) -> ValidationSessionResponse:
        """
        ...
        if not session.can_be_completed():
            raise ValueError(
                f"Session {request.session_id} cannot be completed from status: {session.status}"
            )
        ...
        completed_items = await self._mt_repo.get_session_feedback_count(
            request.session_id
        )
    ...
    async def get_ser_comparison(
        self, request: GetSERComparisonRequest
    ) -> SERComparisonResponse:
        """
        ...
        historical_data_items = await self._mt_repo.get_historical_data_for_comparison(
            request.speaker_id, request.historical_data_ids
        )

        if not historical_data_items:
            raise ValueError(
                f"No historical data found for speaker {request.speaker_id}"
            )
        ...
        for item in historical_data_items:
            ...
            comparison = self._ser_service.compare_ser_metrics(
                original_ser, corrected_ser
            )
                ...
                individual_comparisons.append(
                    {
                        ...
                        "is_significant_improvement": comparison[
        ...
        improvement_percentage = (
            (overall_improvement / float(avg_original_ser)) * 100
            if avg_original_ser > 0
            else 0
        )

        # Generate recommendation
        recommendation = self._generate_bucket_recommendation(
            avg_original_ser, avg_corrected_ser, improvement_percentage
        )
        ...
        comparison_summary = (
            {
                ...
                "significant_improvements": len(
                    [
                        c
                        for c in individual_comparisons
                        if c.get("is_significant_improvement", False)
                    ]
                ),
            }
            if request.include_summary_statistics
            else {}
        )
    ...
    async def _prepare_session_test_data(
        self, session_id: UUID, test_data_ids: List[UUID]
    ) -> None:
        """
    ...
    def _generate_bucket_recommendation(
        ...
        return {
            ...
            "confidence": (
                ...
                else "medium" if improvement_percentage >= 5 else "low"
            ),
        }

    def _session_to_response(
        self, session: ValidationTestSession, completed_items: int
    ) -> ValidationSessionResponse:
        """
