...
class MTValidationFeedback:
    ...
    def _validate_improvement_assessment(self) -> None:
        ...
        if not isinstance(self.improvement_assessment, ImprovementAssessment):
            raise ValueError("improvement_assessment must be a valid ImprovementAssessment")
    ...
    def get_text_length_comparison(self) -> Dict[str, int]:
        ...
        return {
            ...
            "correction_change": len(self.rag_corrected_text) - len(self.original_asr_text),
            "reference_change": len(self.final_reference_text) - len(self.rag_corrected_text)
        }
