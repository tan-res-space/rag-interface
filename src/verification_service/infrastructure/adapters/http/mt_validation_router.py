...
from ....application.use_cases.mt_validation_workflow_use_case import MTValidationWorkflowUseCase
...
async def get_mt_validation_use_case() -> MTValidationWorkflowUseCase:
    ...
    raise HTTPException(status_code=501, detail="MT validation use case not implemented")
...
async def start_validation_session(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to start validation session")
...
@router.get("/sessions/{session_id}/test-data", response_model=List[ValidationTestDataResponse])
async def get_validation_test_data(
    session_id: UUID = Path(..., description="Validation session ID"),
    limit: Optional[int] = Query(None, ge=1, le=100, description="Maximum number of items"),
    use_case: MTValidationWorkflowUseCase = Depends(get_mt_validation_use_case)
):
    """
    ...
    try:
        ...
        logger.info(f"Retrieved {len(response)} test data items for session {session_id}")
        return response
...
@router.post("/sessions/{session_id}/feedback", response_model=MTFeedbackResponse, status_code=201)
...
@router.post("/sessions/{session_id}/complete", response_model=ValidationSessionResponse)
async def complete_validation_session(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to complete validation session")
...
async def get_validation_sessions(
    ...
    """
    try:
        logger.info(f"Getting validation sessions with filters: speaker_id={speaker_id}, status={status}")
...
async def get_session_feedback(
    ...
    try:
        ...
        logger.info(f"Retrieved {len(response)} feedback entries for session {session_id}")
        return response
