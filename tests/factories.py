...
class ErrorReportFactory(factory.Factory):
    ...
    end_position = factory.LazyAttribute(
        lambda obj: min(obj.start_position + fake.random_int(min=1, max=10), len(obj.original_text))
    )
    
    context_notes = factory.LazyFunction(
        lambda: fake.sentence(nb_words=10) if fake.boolean(chance_of_getting_true=70) else None
    )
    ...
    status = factory.Iterator([
        ErrorStatus.PENDING,
        ErrorStatus.PROCESSED,
        ErrorStatus.ARCHIVED
    ])
    
    metadata = factory.LazyFunction(
        lambda: {
            "audio_quality": fake.random_element(elements=["poor", "fair", "good", "excellent"]),
            "background_noise": fake.boolean(),
            "confidence_score": round(fake.random.uniform(0.1, 1.0), 2),
            "speaker_accent": fake.random_element(elements=["american", "british", "australian", "canadian"]),
            "technical_issues": fake.random_elements(
...
class LongTextErrorReportFactory(ErrorReportFactory):
    ...
    original_text = factory.LazyFunction(
        lambda: fake.text(max_nb_chars=5000)
    )
    corrected_text = factory.LazyFunction(
        lambda: fake.text(max_nb_chars=5000)
    )
