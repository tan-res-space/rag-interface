...
class TestPostgreSQLIntegration:
    ...
    async def test_find_by_job_id_multiple_reports(self, repository):
        ...
        job_id = uuid4()
        error_reports = [
            ErrorReportFactory.create(job_id=job_id)
            for _ in range(3)
        ]
    ...
    async def test_search_with_filters_categories(self, repository):
        ...
        grammar_reports = [
            ErrorReportFactory.create(error_categories=["grammar"])
            for _ in range(3)
        ]
    ...
    async def test_search_with_date_range_filter(self, repository):
        ...
        old_reports = [
            ErrorReportFactory.create(
                error_timestamp=base_date - timedelta(days=10)
            )
            for _ in range(2)
        ]
        recent_reports = [
            ErrorReportFactory.create(
                error_timestamp=base_date - timedelta(days=2)
            )
            for _ in range(3)
        ]
