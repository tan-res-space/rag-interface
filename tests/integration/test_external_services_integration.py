...
class TestUserManagementServiceIntegration:
    ...
    async def test_check_user_authorization_success(self, ums_client):
        ...
        with aioresponses.aioresponses() as m:
            ...
            # Act
            is_authorized = await ums_client.check_authorization(user_id, resource, action)
...
class TestNotificationServiceIntegration:
    ...
    async def test_send_critical_error_notification_success(self, notification_client):
        ...
        with aioresponses.aioresponses() as m:
            ...
            # Act
            result = await notification_client.send_critical_error_notification(notification_data)
...
class TestServiceMeshIntegration:
    ...
    async def test_service_discovery_integration(self):
        ...
        # Arrange
        with patch('src.error_reporting_service.infrastructure.adapters.external.service_discovery.ServiceDiscovery') as mock_discovery:
            ...
            # Act
            client = UserManagementServiceClient.from_service_discovery("user-management")
            
            # Assert
            assert "user-management-service.default.svc.cluster.local" in client.base_url
    ...
    async def test_distributed_tracing_headers(self, ums_client):
        ...
        with aioresponses.aioresponses() as m:
            ...
            # Act
            with patch('src.error_reporting_service.infrastructure.adapters.external.tracing.get_current_trace_context') as mock_trace:
    ...
    async def test_load_balancing_across_instances(self):
        ...
        with patch('src.error_reporting_service.infrastructure.adapters.external.load_balancer.LoadBalancer') as mock_lb:
            mock_lb.return_value.get_next_instance.side_effect = service_instances
            
            clients = []
            for _ in range(3):
                client = UserManagementServiceClient.with_load_balancer("user-management")
