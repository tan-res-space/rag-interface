...
class TestKafkaIntegration:
    ...
    def event_publisher(self, mock_kafka_producer, kafka_settings):
        ...
        return KafkaErrorEventPublisher(
            producer=mock_kafka_producer, settings=kafka_settings
        )

    @pytest.mark.asyncio
    async def test_publish_error_reported_event_success(
    ...
    @pytest.mark.asyncio
    async def test_publish_error_updated_event_success(
        self, event_publisher, mock_kafka_producer
    ):
    ...
    @pytest.mark.asyncio
    async def test_publish_error_deleted_event_success(
        self, event_publisher, mock_kafka_producer
    ):
    ...
    @pytest.mark.asyncio
    async def test_publish_event_with_retry_on_failure(
        self, event_publisher, mock_kafka_producer
    ):
    ...
    @pytest.mark.asyncio
    async def test_publish_event_max_retries_exceeded(
        self, event_publisher, mock_kafka_producer
    ):
        ...
        mock_kafka_producer.send.side_effect = Exception(
    ...
    @pytest.mark.asyncio
    async def test_event_serialization_with_complex_metadata(
        self, event_publisher, mock_kafka_producer
    ):
        ...
        assert (
            message["data"]["metadata"]["nested_data"]["recording_device"]
            == "microphone_a"
        )
    ...
    @pytest.mark.asyncio
    async def test_event_ordering_with_partition_key(
        self, event_publisher, mock_kafka_producer
    ):
        ...
        events = [
            ErrorReportedEventFactory.create(speaker_id=speaker_id) for _ in range(3)
        ]
    ...
    @pytest.mark.asyncio
    async def test_event_headers_and_metadata(
        self, event_publisher, mock_kafka_producer
    ):
    ...
    @pytest.mark.asyncio
    async def test_event_compression_and_size_limits(
        self, event_publisher, mock_kafka_producer
    ):
    ...
    async def test_producer_health_check(self, event_publisher, mock_kafka_producer):
        ...
        # Arrange
        mock_kafka_producer.list_topics = AsyncMock(
            return_value={"test_asr.error.reported"}
        )
    ...
    @pytest.mark.asyncio
    async def test_producer_health_check_failure(
        self, event_publisher, mock_kafka_producer
    ):
    ...
    async def test_graceful_shutdown(self, event_publisher, mock_kafka_producer):
        ...
        publish_tasks = [
            event_publisher.publish_error_reported(event) for event in events
        ]
    ...
    async def test_topic_auto_creation_configuration(self, kafka_settings):
        ...
        with patch("aiokafka.AIOKafkaProducer") as mock_producer_class:
            ...
            event_publisher = KafkaErrorEventPublisher(
                producer=mock_producer, settings=kafka_settings
            )
