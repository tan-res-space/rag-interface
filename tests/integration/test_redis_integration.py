...
class TestRedisIntegration:
    ...
    async def test_cache_invalidation_by_pattern(self, cache_adapter):
        ...
        job_id = str(uuid4())
        error_reports = [
            ErrorReportFactory.create(job_id=uuid4())
            for _ in range(3)
        ]
        ...
        # Act
        invalidated_count = await cache_adapter.invalidate_by_pattern(f"search:job_{job_id}:*")
    ...
    async def test_cache_connection_error_handling(self, cache_adapter):
        ...
        with patch.object(cache_adapter.redis_client, 'set', side_effect=ConnectionError("Redis unavailable")):
            # Act & Assert
    ...
    async def test_cache_health_check_failure(self, cache_adapter):
        ...
        with patch.object(cache_adapter.redis_client, 'ping', side_effect=ConnectionError("Redis down")):
            # Act
            is_healthy = await cache_adapter.health_check()
