...
from src.user_management_service.infrastructure.adapters.database.in_memory.adapter import InMemoryUserRepositoryAdapter
from src.user_management_service.infrastructure.adapters.auth.password_service import PasswordServiceAdapter
from src.user_management_service.infrastructure.adapters.auth.token_service import TokenServiceAdapter
from src.user_management_service.infrastructure.adapters.messaging.in_memory.adapter import InMemoryEventPublisherAdapter
...
class TestUMSIntegration:
    ...
    @pytest.fixture
    async def authenticate_use_case(self, user_repository, password_service, 
                                   token_service, event_publisher, validation_service):
    ...
    @pytest.mark.asyncio
    async def test_complete_authentication_workflow(self, authenticate_use_case, 
                                                   test_user, event_publisher):
    ...
    @pytest.mark.asyncio
    async def test_authentication_with_invalid_password(self, authenticate_use_case,
                                                       test_user, event_publisher):
        ...
        published_events = event_publisher.get_published_events()
        failure_events = [e for e in published_events if e["event_type"] == "user.login_failure"]
        assert len(failure_events) == 1
    
    @pytest.mark.asyncio
    async def test_authentication_with_nonexistent_user(self, authenticate_use_case,
                                                       event_publisher):
    ...
    async def test_password_service_operations(self, password_service):
        ...
        strength = await password_service.calculate_password_strength(generated_password)
        assert strength > 0.7
    ...
    async def test_event_publisher_operations(self, event_publisher):
        ...
        from src.user_management_service.domain.events.domain_events import UserCreatedEvent
