...
class TestSearchErrorsUseCase:
    ...
    async def test_execute_search_with_date_range_filter(self):
        ...
        recent_errors = [
            ErrorReportFactory.create(error_timestamp=start_date + timedelta(days=i))
            for i in range(3)
        ]

        request = SearchErrorsRequest(
            requested_by=user_id,
            filters=ErrorFilters(date_from=start_date, date_to=end_date),
            pagination=PaginationParams(page=1, size=10),
    ...
    async def test_execute_search_with_text_search(self):
        ...
        for error in response.results.items:
            assert (
                "diabetes" in error.original_text.lower()
                or "diabetes" in error.corrected_text.lower()
            )
