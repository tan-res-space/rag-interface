...
class TestPostgreSQLAdapter:
    ...
    def setup_method(self):
        ...
        with patch('src.error_reporting_service.infrastructure.adapters.database.postgresql.adapter.create_async_engine'):
            with patch('src.error_reporting_service.infrastructure.adapters.database.postgresql.adapter.async_sessionmaker'):
    ...
    async def test_save_error_report_success(self):
        ...
        with patch.object(self.adapter, '_session_factory', mock_session_factory):
            with patch('src.error_reporting_service.infrastructure.adapters.database.postgresql.models.ErrorReportModel', return_value=mock_model):
                with patch.object(self.adapter, '_model_to_entity', return_value=error_report):
    ...
    async def test_find_errors_by_speaker(self):
        ...
        with patch.object(self.adapter, '_session_factory', return_value=mock_session):
            with patch.object(self.adapter, '_model_to_entity', side_effect=error_reports):
