...
run_quality_checks() {
    ...
    mkdir -p reports/test-results/quality
    ...
    flake8 src/ tests/ \
        ...
        --format=json \
        --output-file=reports/test-results/quality/flake8-report.json || true

    # Type checking with mypy
    log_info "Running mypy type checking..."
    mypy src/ \
        --ignore-missing-imports \
        --junit-xml=reports/test-results/quality/mypy-report.xml || true

    # Security scanning with bandit
    log_info "Running bandit security scan..."
    bandit -r src/ \
        -f json \
        -o reports/test-results/quality/bandit-report.json || true

    # Code formatting check with black
    log_info "Checking code formatting with black..."
    black --check --diff src/ tests/ > reports/test-results/quality/black-report.txt || true

    # Import sorting check with isort
    log_info "Checking import sorting with isort..."
    isort --check-only --diff src/ tests/ > reports/test-results/quality/isort-report.txt || true
...
generate_backend_report() {
    ...
    cat > reports/test-results/backend-report.md << EOF
...
## Code Quality
$([ -f "reports/test-results/quality/flake8-report.json" ] && echo "- Linting Report: [Flake8 JSON](quality/flake8-report.json)" || echo "- Linting: Not run")
$([ -f "reports/test-results/quality/mypy-report.xml" ] && echo "- Type Checking: [MyPy XML](quality/mypy-report.xml)" || echo "- Type Checking: Not run")
$([ -f "reports/test-results/quality/bandit-report.json" ] && echo "- Security Scan: [Bandit JSON](quality/bandit-report.json)" || echo "- Security Scan: Not run")

## Service-Specific Coverage
$(find reports/test-results -name "coverage-*.xml" | while read file; do
...
EOF
    ...
    log_success "Backend test report generated: reports/test-results/backend-report.md"
}
